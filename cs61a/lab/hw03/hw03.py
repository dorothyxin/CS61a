...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(first,index,count):
        if n == index:
            return first
        if num_eights(index) > 0 or index % 8 == 0:
            return helper(first - count, index+1, -count)
        else:
            return helper(first + count, index+1,count)
    return helper(1,1,1)
...
def missing_digits(n):
    ...
    def helper(gap,x,n):
        if n == 0:
            return gap
        if x - n%10 > 0:
            return helper(gap + x - n%10 - 1, n%10,n//10)
        else:
            return helper(gap,n%10,n//10)
    return helper(0,0,n)
...
def count_coins(change):
    ...
    def count_partition(change,m):
        if change == 0:
            ...
            return count_partition(change-m,m) + count_partition(change,descending_coin(m))
