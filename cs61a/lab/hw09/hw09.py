...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r"(\b[IXVCLMD]+\b)", text)
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r"([Cc][Ss]\s?[0-9]+[ABCabc]?)", post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r"[2][0123]:[012345][0-9][APap]*[Mm]*|[01]?[0-9]:[012345][0-9][APap]*[Mm]*", text)
