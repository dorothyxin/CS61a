...
def balanced(m):
    ...
    if length(left(m))*total_weight(end(left(m))) == length(right(m))*total_weight(end(right(m))):
        ...
        if is_mobile(end(right(m))) and not is_mobile(end(right(m))):
...
def totals_tree(m):
    ...
    if is_planet(l) and is_planet(r):
        return tree(w,[tree(total_weight(l)),tree(total_weight(r))])
    if is_planet(l) and not is_planet(r):
        return tree(w,[tree(total_weight(l)),totals_tree(r)])
    if not is_planet(l) and is_planet(r):
        return tree(w,[totals_tree(l),tree(total_weight(r))])
    if not is_planet(l) and not is_planet(r):
        return tree(w,[totals_tree(l),totals_tree(r)])
