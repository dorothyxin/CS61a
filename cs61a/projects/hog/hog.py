    ...
    def announce(score0,score1,last_score=last_score,running_high=running_high):
        if who == 0:
            if score0-last_score>running_high:
                print("{0} point(s)! That's a record gain for Player {1}!".format(score0-last_score,who))
            ...
            if score1-last_score>running_high:
                print("{0} point(s)! That's a record gain for Player {1}!".format(score1-last_score,who))
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averagedice(*args):
        ...
        for i in range(trials_count):
            j += original_function(*args)
        return j/trials_count
    return averagedice
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max = 0
    while num <= 10:
        cur = make_averaged(roll_dice,trials_count=trials_count)(num,dice)
        if cur >= max:
            max = cur
            maxnum = num
        num += 1
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if hog_pile(newscore,opponent_score):
        return 0
    else:
