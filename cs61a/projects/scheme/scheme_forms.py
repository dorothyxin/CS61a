...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        t = expressions.rest.first
        env.define(signature,scheme_eval(t,env))
        return signature
        # END PROBLEM 4
        ...
        er = expressions.rest
        pr = Pair(sr,er)
        procedure = do_lambda_form(pr, env)
        env.define(sf, procedure)
        return sf
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    for i in range(len(bindings)):
        b = bindings.first
        validate_form(b, 2, 2)
        evl=scheme_eval(b.rest.first, env)
        values = Pair(evl, values)
        name1=Pair(b.first, names)
        names = name1
