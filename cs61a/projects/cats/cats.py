...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def helper(phase):
        phase = remove_punctuation(phase)
        res = []
        if type(phase) != list:
            res.append(phase)
            phase = res
...
def accuracy(typed, reference):
    ...
    if typed_words == [] and reference_words != []:
        return 0.0
    if reference_words == [] and typed_words != []:
        return 0.0
    else:
        count = 0
        i = 0
        n =len(typed_words)
        l = len(reference_words)
        while i < n and i < l:
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    f = [[] for i in player_indices]
    for w in word_indices:
        mintime = float("inf")
        player = 0
        for p in player_indices:
            if time(match, p, w) < mintime:
                mintime = time(match, p, w)
                player = p
        f[player].append(word_at(match,w))
    return f
